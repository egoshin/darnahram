...
if (!empty($arResult["ERROR_MESSAGE"])):
	?>
	<div class="photo-error" xmlns="http://www.w3.org/1999/html">
...
<div class="row" id="photo_list_<?= htmlspecialcharsEx($arParams["~UNIQUE_COMPONENT_ID"])?>" style="width: auto !important;">
	...
	foreach ($arResult["ELEMENTS_LIST"] as $key => $arItem)
	{
		...
		?>
		<article class="white-panel" id="photo_cont_<?=$arItem["ID"]?>"  title="<?= $arItem["TITLE"]?>">
				<a class="photo-item-inner hidden-xs hidden-sm" style="width: <?= $arParams['THUMBNAIL_SIZE']?>px; height: <?= $arParams['THUMBNAIL_SIZE']?>px;" href="<?=$arItem["URL"]?>" id="photo_<?=$arItem["ID"]?>">
					<img class="img-responsive" src="<?= $src?>" style="<?= $item_w?><?= $item_left?> <?= $item_top?>;" alt="<?= $alt?>"/>
					<?if($bNotActive):?>
				...
				<img class="hidden-md hidden-lg img-responsive" src="<?= $arItem["BIG_PICTURE"]["SRC"]?>" alt="<?= $alt?>"/>
		</article>
		<?
	};?>
	</section>
